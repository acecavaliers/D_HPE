    ...
    <MasterLayout>
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <div class="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 bg-white">
                ...
                <div class="px-5">
                    <div>
                        <button @click="showModal = true" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">Add Physical Exam</button>
                        <Modal :title="'Add Physical Exam'" :isVisible="showModal" @close="handleModalClose">
                            <AddPE @formSubmitted="handleFormSubmitted"/>
                        </Modal>
                    </div>
                </div>
            </div>
            <div class="">
                <PEList :key="Pe_ListKey"/>
            </div>
        </div>
</MasterLayout>
...
import AddPE from '@/Pages/PhysicalExam/AddPE.vue';
import PEList from '@/Pages/PhysicalExam/PEList.vue';

const showModal = ref(false);

const Pe_ListKey = ref(0);
