<template>
      ...
      <div class="relative overflow-x-auto border">
        <table class="w-full text-sm text-left text-gray-500 p-2 uppercase ">
            ...
            <tr v-for="(item, index) in records.data" :key="index" class="bg-white border-b hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                <span v-if="item.with_input == 1" class="inline-flex items-center gap-x-1.5 rounded-full bg-green-100 px-2 py-1 text-xs font-medium text-green-700">
  ...
  const fetchRecords = async (url = null) => {
    try {
      const response = await axios.get(url || route('fh.getlist'));
